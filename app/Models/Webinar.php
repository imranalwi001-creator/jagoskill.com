...
class Webinar extends Model implements TranslatableContract
{
    ...
    public function getRate()
    {
        ...
        if (!empty($this->avg_rates)) {
            $rate = $this->avg_rates;
        } elseif (isset($this->active_reviews_avg_rates)) {
            $rate = $this->active_reviews_avg_rates;
        } elseif ($this->relationLoaded('reviews')) {
            $reviews = $this->reviews->where('status', 'active');
    ...
    {
        if (isset($this->active_reviews_count)) {
            return $this->active_reviews_count;
        }
